...
public class UserController extends Controller {
	...
	public static Result getRequestsByUser(Integer id) {
		List<TripRequest> requests = TripRequest.find.where().eq("user.id", id)
				.findList();
		// TripRequest request = TripRequest.find.where().eq("user.id",
		// id).findList().get(0);

		JSONSerializer serializer = new JSONSerializer().exclude(
				"matches.tripRequest.matches", "matches.tripOffer.matches",
				"*.password");
	...
	public static Result getOffersByUser(Integer id) {
		...
		List<TripOffer> offers = TripOffer.find.where().eq("user.id", id)
				.findList();
	...
	public static Result getMatchesByUser(Integer id) {
		List<TripMatch> matches = TripMatch.find.where()
				.eq("tripOffer.user.id", id).findList();

		JSONSerializer serializer = new JSONSerializer().exclude(
				"tripRequest.matches", "tripOffer.matches", "*.password");
	...
	public static Result searchUserByName(String name) {
		List<User> users = User.find.where().like("email", "%" + name + "%")
				.findList();
	...
	public static Result getNotifications() {
		List<Notification> notifications = Notification.find.where()
				.eq("user", activeUser()).findList();

		JSONSerializer serializer = new JSONSerializer()
				.exclude("tripMatch.tripRequest.matches").exclude("*.password")
				.include("*");
		response().setContentType("application/json");
	...
	public static Result getRatingUp(Integer id) {
		...
		JSONSerializer serializer = new JSONSerializer().exclude("*.password")
				.include("*");
		response().setContentType("application/json");
